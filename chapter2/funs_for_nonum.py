...
def fourier_Hilbert_3D_for2(indata,nlons,peak_freq=0,ndegs=360):
...
def fourier_Tukey_hilow_3D_for2(indata,nlons,s1,s2,ndegs=360):
    try:
...
def calc_wavepacket_3D_for2(datain,len_min,len_max):
    # Subtract zonal mean
...
#                               dims = ('time','latitude','longitude'))
    ...
    for latsel in np.arange(ilatstart,ilatend+1):
        ...
        tukey_transform, std_transform,t = fourier_Tukey_hilow_3D_for2(indata2,nlons,s1,s2,ndegs=360)
        # Now hilbert transfrom (For now, ignoring the semi-geostrophic filter)
        hilbert_transform,t = fourier_Hilbert_3D_for2(tukey_transform,nlons)
